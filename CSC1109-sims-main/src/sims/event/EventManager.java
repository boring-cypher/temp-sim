...
public class EventManager {
    ...
    private final List<Events> examEvents; // to call executeChoice
    ...
    private void printPhaseTransition(Phase previousPhase, Phase currentPhase, sims.character.Character character) {
    ...
    private List<Events> getEventsForTurn(Phase currentPhase, int turnNo) {
        List<Events> phaseEvents = switch (currentPhase) {
            case Phase.STUDY -> studyEvents;
            case Phase.RECESS -> holidayEvents;
            case Phase.EXAM -> examEvents;
        };
    ...
    public void mainGameFlow(Character character) {
        ...
        for (int currentTurn = 1; currentTurn <= TOTAL_TURNS; currentTurn++) {
            ...
            turnProcessor.playTurn(character, currentTurn, MAX_ENERGY, eventsThisTurn, currentPhase, randomEvents);
